//
//  RegistrationCodeController.swift
//  YummPizza
//
...
class RegistrationCodeController: UIViewController
{
   ...
   {
      RequestManager.registration(phoneNumber: registrationPhoneNumber, smsCode: code,
      success:
      {
         smsToken in
         registrationSmsToken = smsToken
         self.performSegue(withIdentifier: "RegistrationProfile", sender: self)
      },
      failure:
      {
         errorDescription in
         ...
         case "ERROR_USER_WITH_THIS_PHONE_ALREADY_EXIST": AlertManager.showAlert("Номер уже зарегистрирован в системе")
   ...
   func resendPhone()
   {
      RequestManager.registration(phoneNumber: registrationPhoneNumber,
      success:
      {
